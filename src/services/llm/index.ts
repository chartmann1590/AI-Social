...
async function generateAtLeastPosts(
  ...
  while (all.length < target && attempts < MAX_FEED_ATTEMPTS) {
    ...
    try {
      const batch = await runOne(Math.max(need, 3));
...
export const LlmService = {
  async generateFeedPosts(settings: AppSettings, count: number = 5): Promise<Post[]> {
    ...
    return withFallback(
      settings.llmMode,
      settings,
      () => generateAtLeastPosts(settings, target, (n) => localLiteRtProvider.generateFeedPosts(settings, n)),
      () => generateAtLeastPosts(settings, target, (n) => remoteOllamaProvider.generateFeedPosts(settings, n)),
    );
  },
