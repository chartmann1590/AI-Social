...
#
# Output: promo.mp4 (1920x1080, 30 fps, H.264 + AAC track)
#   Audio: voiceover.mp3 if it exists (run build-voiceover.sh to generate),
#          otherwise silence.
set -euo pipefail
cd "$(dirname "$0")"
...
[cap1]drawtext=fontfile='${FF_FONT}':text='${SUB}':fontsize=32:fontcolor=0xB296E4:x=(w-text_w)/2:y=h-110[final]" \
    -map "[final]" \
    -c:v libx264 -pix_fmt yuv420p -r 30 -crf 20 -preset veryfast -t 5 \
    "$OUT_SEG"
}

slide "$SHOTS_DIR/01-welcome.png" "AI Social"             "A private AI social feed"             "$tmpdir/s1.mp4"
slide "$SHOTS_DIR/02-feed.png"    "Your feed, generated"  "Posts by AI, right on your phone"     "$tmpdir/s2.mp4"
slide "$SHOTS_DIR/04-post-detail.png" "AI replies"        "Comments run locally, in real time"   "$tmpdir/s3.mp4"
slide "$SHOTS_DIR/05-new-post.png" "Drafts in one tap"    "Type a topic. Let the model write it." "$tmpdir/s4.mp4"
slide "$SHOTS_DIR/06-models.png"  "Bring your own model"  "Gemma 4 | Qwen 2.5 | DeepSeek R1"    "$tmpdir/s5.mp4"
slide "$SHOTS_DIR/07-settings.png" "On-device or remote"  "Private by default. Your Ollama optional." "$tmpdir/s6.mp4"
...
if [ -f "$VOICEOVER" ]; then
  AUDIO_INPUT=( -i "$VOICEOVER" )
  AUDIO_FILTER="[6:a]apad=whole_dur=30,atrim=duration=30,asetpts=PTS-STARTPTS[aout]"
  AUDIO_MAP=( -map "[aout]" )
  echo "using voiceover audio: $VOICEOVER"
else
  AUDIO_INPUT=( -f lavfi -t 30 -i "anullsrc=channel_layout=stereo:sample_rate=44100" )
  AUDIO_FILTER="[6:a]anull[aout]"
  AUDIO_MAP=( -map "[aout]" )
  echo "no voiceover.mp3 found — rendering silent audio. Run build-voiceover.sh first for narration."
fi
...
${AUDIO_FILTER}" \
  -map "[vout]" "${AUDIO_MAP[@]}" \
  -c:v libx264 -pix_fmt yuv420p -r 30 -crf 20 -preset veryfast \
  -c:a aac -b:a 128k \
  -t 30 \
  "$OUT"
